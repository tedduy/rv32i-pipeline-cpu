...
module tb_zicntr;
    ...
    initial begin
        ...
        if (dut.id_regfile.regs[2] <= dut.id_regfile.regs[1])
            $fatal(1, "cycle did not increase: first=%08h second=%08h",
                   dut.id_regfile.regs[1], dut.id_regfile.regs[2]);
        if ((dut.id_regfile.regs[4] - dut.id_regfile.regs[3]) !== 32'd4)
            $fatal(1, "instret delta=%0d, expected four preceding instructions",
                   dut.id_regfile.regs[4] - dut.id_regfile.regs[3]);
        if (dut.id_regfile.regs[5] == 32'b0)
            $fatal(1, "time CSR did not reflect the advancing platform timebase");
        if (dut.id_regfile.regs[6] !== TIME_HIGH)
            $fatal(1, "timeh=%08h, expected %08h",
                   dut.id_regfile.regs[6], TIME_HIGH);
        if (dut.id_regfile.regs[8] !== COUNTER_HIGH)
            $fatal(1, "cycleh=%08h, expected %08h",
                   dut.id_regfile.regs[8], COUNTER_HIGH);
        if (dut.id_regfile.regs[9] !== COUNTER_HIGH)
            $fatal(1, "instreth=%08h, expected %08h",
                   dut.id_regfile.regs[9], COUNTER_HIGH);
