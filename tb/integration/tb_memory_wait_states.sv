...
module tb_memory_wait_states;
    ...
    initial begin
        ...
        if (dut.id_regfile.regs[7] !== 32'd12)
            $fatal(1, "Arithmetic before memory failed: x7=%08h", dut.id_regfile.regs[7]);
        if (dut.id_regfile.regs[8] !== 32'd12)
            $fatal(1, "Delayed load failed: x8=%08h", dut.id_regfile.regs[8]);
        if (dut.id_regfile.regs[9] !== 32'd13)
            $fatal(1, "Post-load dependency failed: x9=%08h", dut.id_regfile.regs[9]);
