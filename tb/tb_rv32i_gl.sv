...
module tb_rv32i_gl;
    ...
    always_comb begin
        automatic int idx = W_PC_out[8:2];
    ...
    initial begin
        ...
        repeat(120) begin
            ...
            if (W_PC_out != last_pc && W_PC_out != 0 && cycle_count > 5) begin
                automatic int alu_idx = (W_PC_out - 4) / 4;  // Index for ALU expected
                automatic logic [N-1:0] exp_alu;
                automatic logic alu_match;
                automatic string instr_type;
                
                // Determine instruction type based on PC
                if (W_PC_out >= 32'h04 && W_PC_out <= 32'h50) 
                    instr_type = "R-Type";
                else if (W_PC_out >= 32'h54 && W_PC_out <= 32'h9C) 
                    instr_type = "I-Type";
                else if (W_PC_out >= 32'hA0 && W_PC_out <= 32'hC4) 
                    instr_type = "Load";
                else if (W_PC_out >= 32'hC8 && W_PC_out <= 32'hDC) 
                    instr_type = "Store";
                else if (W_PC_out >= 32'hE0 && W_PC_out <= 32'h10C) 
                    instr_type = "Branch";
                else if (W_PC_out >= 32'h110 && W_PC_out <= 32'h11C) 
                    instr_type = "U-Type";
                else if (W_PC_out >= 32'h120 && W_PC_out <= 32'h130) 
                    instr_type = "JAL/JALR";
                else 
                    instr_type = "Other";
                ...
                if (alu_idx >= 0 && alu_idx <= 76) begin
                    ...
                    alu_match = (W_ALUout == exp_alu);
                    
                    if (alu_match) begin
                        pass_count++;
                        $display("%6d | 0x%08h | 0x%08h | 0x%08h | %-8s | PASS   | %s", 
                                 cycle_count, W_PC_out, W_ALUout, exp_alu, "YES", instr_type);
                    end else begin
                        fail_count++;
                        $display("%6d | 0x%08h | 0x%08h | 0x%08h | %-8s | FAIL   | %s", 
                                 cycle_count, W_PC_out, W_ALUout, exp_alu, "NO", instr_type);
                    end
                end else begin
                    // Beyond expected range
                    $display("%6d | 0x%08h | 0x%08h | %-10s | %-8s | ---    | %s", 
                             cycle_count, W_PC_out, W_ALUout, "N/A", "---", instr_type);
                end
