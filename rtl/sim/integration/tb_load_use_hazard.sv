...
module tb_load_use_hazard;
    ...
    initial begin
        ...
        if (dut.u_id_regfile.regs[3] !== 32'd32)
            $fatal(1, "LW failed: x3=%08h, expected 00000020", dut.u_id_regfile.regs[3]);

        if (dut.u_id_regfile.regs[6] !== 32'd48)
            $fatal(1, "Load-use result failed: x6=%08h, expected 00000030", dut.u_id_regfile.regs[6]);

        if (dut.u_id_regfile.regs[7] !== 32'd49)
            $fatal(1, "Post-stall forwarding failed: x7=%08h, expected 00000031", dut.u_id_regfile.regs[7]);
