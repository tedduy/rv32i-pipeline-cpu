...
module tb_bus_access_faults;
    ...
    initial begin
        ...
        wait (dut.u_id_regfile.regs[13] == 32'd1);
        repeat (4) @(posedge clk);
        #1;

        if (trap_count !== 3 || dut.u_id_regfile.regs[7] !== 32'd3)
            $fatal(1, "Observed %0d traps, handler count x7=%0d",
                   trap_count, dut.u_id_regfile.regs[7]);
        if (dut.u_id_regfile.regs[11] !== 32'd0)
            $fatal(1, "Faulting load modified x11=%08h", dut.u_id_regfile.regs[11]);
        if (dut.u_id_regfile.regs[10] !== 32'd1 ||
            dut.u_id_regfile.regs[12] !== 32'd1 ||
            dut.u_id_regfile.regs[13] !== 32'd1)
