...
module tb_machine_csr_trap;
    ...
    initial begin
        ...
        wait (dut.u_id_regfile.regs[3] == 32'd7);
        repeat (3) @(posedge clk);
        #1;

        if (dut.u_id_regfile.regs[2] !== 32'd5)
            $fatal(1, "x2=%08h, expected 00000005", dut.u_id_regfile.regs[2]);
        if (dut.u_id_regfile.regs[3] !== 32'd7)
            $fatal(1, "MRET did not return to PC 0x18");
        if (dut.u_id_regfile.regs[4] !== 32'h0000_0018)
            $fatal(1, "Trap handler computed mepc=%08h, expected 00000018",
                   dut.u_id_regfile.regs[4]);
        if (dut.u_id_regfile.regs[5] !== 32'd0)
            $fatal(1, "CSRRWI did not return old mscratch value");
        if (dut.u_id_regfile.regs[6] !== 32'd3)
            $fatal(1, "CSRRS read mscratch=%08h, expected 00000003",
                   dut.u_id_regfile.regs[6]);
        if (dut.u_machine_csrs.mcause !== 32'd11)
            $fatal(1, "mcause=%08h, expected machine ECALL cause 11",
                   dut.u_machine_csrs.mcause);
        if (dut.u_machine_csrs.mepc !== 32'h0000_0018)
            $fatal(1, "mepc=%08h, expected handler-updated value 00000018",
                   dut.u_machine_csrs.mepc);
