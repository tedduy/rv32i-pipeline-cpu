...
module tb_zmmul;
    ...
    initial begin
        ...
        wait (dut.u_id_regfile.regs[11] == 32'd7);
        repeat (3) @(posedge clk);
        #1;

        if (dut.u_id_regfile.regs[3] !== 32'hffff_fffa)
            $fatal(1, "MUL result=%08h, expected fffffffa", dut.u_id_regfile.regs[3]);
        if (dut.u_id_regfile.regs[4] !== 32'hffff_ffff)
            $fatal(1, "MULH result=%08h, expected ffffffff", dut.u_id_regfile.regs[4]);
        if (dut.u_id_regfile.regs[5] !== 32'hffff_ffff)
            $fatal(1, "MULHSU result=%08h, expected ffffffff", dut.u_id_regfile.regs[5]);
        if (dut.u_id_regfile.regs[6] !== 32'h0000_0002)
            $fatal(1, "MULHU result=%08h, expected 00000002", dut.u_id_regfile.regs[6]);
        if (dut.u_id_regfile.regs[9] !== 32'd63 ||
            dut.u_id_regfile.regs[10] !== 32'd64)
            $fatal(1, "MUL forwarding failed: x9=%08h x10=%08h",
                   dut.u_id_regfile.regs[9], dut.u_id_regfile.regs[10]);
