...
module tb_machine_external_interrupt;
    ...
    initial begin
        irq_external = 1'b1;
        wait (dut.u_machine_csrs.mcause == 32'h8000_000b);
        @(negedge clk);
        irq_external = 1'b0;
    end
    ...
    initial begin
        ...
        wait (dut.u_id_regfile.regs[7] == 32'd7);
        repeat (3) @(posedge clk);
        #1;

        if (dut.u_machine_csrs.mcause !== 32'h8000_000b)
        ...
        if (dut.u_id_regfile.regs[5] !== 32'd9)
            $fatal(1, "Interrupt handler did not execute");
        if (dut.u_id_regfile.regs[2] !== 32'd1 || dut.u_id_regfile.regs[3] !== 32'd1)
        ...
        if (!dut.u_machine_csrs.mstatus_mie)
