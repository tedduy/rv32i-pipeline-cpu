module data_memory #(
  parameter N      = 32,
  parameter BYTES  = 256
)(
  input  logic         i_clk,
  input  logic         i_arst_n,
  input  logic         i_we,        // write enable
  input  logic         i_re,        // read enable
  input  logic [N-1:0] i_addr,      // Byte address
  input  logic [N-1:0] i_wdata,     // Little-endian write data from the LSU
  input  logic  [3:0]  i_wstrb,     // Byte enables for SB/SH/SW
  output logic [N-1:0] o_rdata      // Raw word returned to the LSU
);

  // Byte-addressed memory (little-endian)
  logic [7:0] mem [0:BYTES-1];

  // Align reads to the containing 32-bit word.
  wire [N-1:0] addr_a = { i_addr[N-1:2], 2'b00 };

  integer i;
  always_ff @(posedge i_clk or negedge i_arst_n) begin
    if (!i_arst_n) begin
      for (i = 0; i < BYTES; i++) mem[i] <= '0;
    end else if (i_we) begin
      // Apply byte write enables independently.
      if (i_wstrb[0]) mem[addr_a + 0] <= i_wdata[7:0];
      if (i_wstrb[1]) mem[addr_a + 1] <= i_wdata[15:8];
      if (i_wstrb[2]) mem[addr_a + 2] <= i_wdata[23:16];
      if (i_wstrb[3]) mem[addr_a + 3] <= i_wdata[31:24];
    end
  end

  // The LSU performs byte/half selection and sign extension.
  always_comb begin
    o_rdata = '0;
    if (i_re) begin
      o_rdata = { mem[addr_a + 3],
                  mem[addr_a + 2],
                  mem[addr_a + 1],
                  mem[addr_a + 0] };
    end
  end
endmodule
